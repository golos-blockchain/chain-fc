...
namespace fc {
    namespace json_relaxed {
        ...
        std::string tokenFromStream(T &in) {
            ...
            FC_RETHROW_EXCEPTIONS(warn, "while parsing token '${token}'", ("token", token.str()));
        }
            ...
            try {
                    ...
                    case '\'':
                        if (strict)
                            FC_THROW_EXCEPTION(parse_error_exception,
                                               "expected: '\"' at beginning of std::string, got '\''");
                        // falls through
                    case '"':
                        break;
                    default:
                        if (strict)
                            FC_THROW_EXCEPTION(parse_error_exception, "expected: '\"' at beginning of std::string");
                        else
                            FC_THROW_EXCEPTION(parse_error_exception,
                                               "expected: '\"' | '\\\'' at beginning of std::string");
                }
                if (in.peek() == q) {
                    in.get();
                    if (in.peek() != q) {
                    ...
                    if (strict)
                        ...
                        while (true) {
                            ...
                            if (c == q) {
                                ...
                                FC_THROW_EXCEPTION(parse_error_exception, "unexpected EOF in std::string '${token}'",
                                                   ("token", token.str()));
                ...
                while (true) {
                    ...
                    if (c == q) {
                        ...
                        FC_THROW_EXCEPTION(parse_error_exception, "unexpected EOL in std::string '${token}'",
                                           ("token", token.str()));
                    else {
                        in.get();
                        token << c;
        ...
        std::string stringFromStream(T &in) {
            try {
                ...
                switch (c) {
                    ...
                    default:
                        FC_THROW_EXCEPTION(parse_error_exception, "expected: std::string");
                }

            } FC_RETHROW_EXCEPTIONS(warn, "while parsing std::string");
        }
        ...
        fc::variant parseInt(const std::string &token, size_t start) {
            ...
            while (true) {
                ...
                uint8_t vc = ctbl[c];
                if (vc == 0xFF)
                    FC_THROW_EXCEPTION(parse_error_exception, "illegal character {c} in integer of base {b}",
                                       ("c", c)("b", base));
                if (val > maxb4mul)
        ...
        fc::variant parseNumberOrStr(const std::string &token) {
            try {
                ...
                switch (c) {
                    case '0':
                        ...
                        switch (token[i]) {
                            ...
                            default:
                                ...
                                if (strict)
                                    FC_THROW_EXCEPTION(parse_error_exception,
                                                       "expected '.'|'e'|'E' parsing number, got '{c}'", ("c", c));
                        }
                        break;
                    case '1':
                    case '2':
                    ...
                    case '/':
                        if (strict)
                            FC_THROW_EXCEPTION(parse_error_exception, "illegal character '{c}' parsing number",
                                               ("c", c));
                        return fc::variant(token);
                    default:
                    ...
                    switch (c) {
                            ...
                            if (dot_ok) {
                                ...
                                switch (c) {
                                    ...
                                    case '/':
                                        ...
                                        return fc::variant(token);
                                    default:
                                        FC_THROW_EXCEPTION(parse_error_exception, "illegal character '{c}' in token",
                                                           ("c", c)("i", int(c)));
                                }
                            } else {
                                if (strict)
                        ...
                        case 'E':
                            ...
                            switch (c) {
                                ...
                                case '/':
                                    if (strict)
                                        FC_THROW_EXCEPTION(parse_error_exception, "illegal character '{c}' in number",
                                                           ("c", c));
                                    return fc::variant(token);
                                default:
                                    FC_THROW_EXCEPTION(parse_error_exception, "illegal character '{c}' in token",
        ...
        variant_object objectFromStream(T &in) {
            ...
            try {
                char c = in.peek();
                if (c != '{')
                    FC_THROW_EXCEPTION(parse_error_exception, "Expected '{', but read '${char}'",
                ...
                while (in.peek() != '}') {
                    ...
                    skip_white_space(in);
                    if (in.peek() != ':') {
                        FC_THROW_EXCEPTION(parse_error_exception, "Expected ':' after key \"${key}\"", ("key", key));
                    }
                    in.get();
        ...
        variants arrayFromStream(T &in) {
            ...
            try {
                ...
                if (in.peek() != ']')
                    FC_THROW_EXCEPTION(parse_error_exception, "Expected ']' after parsing ${variant}", ("variant", ar));

                in.get();
            } FC_RETHROW_EXCEPTIONS(warn, "Attempting to parse array ${array}", ("array", ar));
            return ar;
        }
            ...
            while (signed char c = in.peek()) {
                switch (c) {
                    ...
                    case '9':
                        ...
                        // null, true, false, or 'warning' / std::string
                    case 'a':
                    case 'b':
                    case 'c':
                    case 'd':
