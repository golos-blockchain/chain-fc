...
namespace fc {
    ...
    /** Decodes utf 8 std::string into unicode std::string.
        @param input   - input std::string to be decoded and stored in 'storage'
        @param storage - buffer for converted text. Cannot be nullptr.
    */
    void decodeUtf8(const std::string &input, std::wstring *storage);

    /** Encodes given wide (unicode) std::string into UTF-8 representation.
        @param input - input std::string to be encoded and stored in 'storage'
        @param storage - buffer for converted text. Cannot be nullptr.
    */
