...
#include <fc/reflect/stringize.hpp>
...
namespace fc {  \
  template<> struct get_typename<FC_REMOVE_PARENTHNESS(TYPE)>  { static const char* name()  {\
 return FC_STRINGIZE(FC_REMOVE_PARENTHNESS(TYPE));}}; \
template<> struct reflector<FC_REMOVE_PARENTHNESS(TYPE)> {\
    typedef FC_REMOVE_PARENTHNESS(TYPE) type; \
    typedef fc::true_type  is_defined; \
    typedef fc::false_type is_enum; \
...
namespace fc {  \
  template<BOOST_PP_SEQ_ENUM(TEMPLATE_ARGS)> struct get_typename<FC_REMOVE_PARENTHNESS(TYPE)>  { static const char* name()  { return FC_STRINGIZE(FC_REMOVE_PARENTHNESS(TYPE));}  }; \
template<BOOST_PP_SEQ_ENUM(TEMPLATE_ARGS)> struct reflector<FC_REMOVE_PARENTHNESS(TYPE)> {\
    typedef FC_REMOVE_PARENTHNESS(TYPE) type; \
    typedef fc::true_type  is_defined; \
    typedef fc::false_type is_enum; \
    enum  member_count_enum {  \
