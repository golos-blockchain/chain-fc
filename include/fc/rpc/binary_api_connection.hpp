...
namespace fc {
    ...
    namespace detail {
        ...
        public:
            ...
            callback_functor(std::weak_ptr<fc::binary_api_connection> con, uint64_t id) : _callback_id(id),
                    _binary_api_connection(con) {
            }
        ...
        R call_generic(const std::function<R(Arg0, Args...)> &f, variants::const_iterator a0,
            ...
            return call_generic<R, Args...>(
                    bind_first_arg<R, Arg0, Args...>(f, a0->as<typename std::decay<Arg0>::type>()), a0 + 1, e);
        }
        ...
        public:
            any_api(api_id_type api_id, const std::shared_ptr<fc::binary_api_connection> &con) : _api_id(api_id),
                    _binary_api_connection(con) {
            }
    ...
    public:
        ...
        vector<char> call(const std::string &name, const vector<char> &args) {
    ...
    private:
        ...
        R call_generic(const std::function<R(std::function<Signature>, Args...)> &f, datastream<const char *> &ds) {
            ...
            return call_generic<R, Args...>(
                    this->bind_first_arg<R, std::function<Signature>, Args...>(f, std::function<Signature>(arg0)), ds);
        }
        ...
        R call_generic(const std::function<R(const std::function<Signature> &, Args...)> &f,
            ...
            fc::raw::unpack(ds, callback_id);
            detail::callback_functor<Signature> arg0(get_connection(), callback_id);
            return call_generic<R, Args...>(this->bind_first_arg<R, const std::function<Signature> &, Args...>(f, arg0),
                                            ds);
        }
        ...
        struct api_visitor {
            ...
            std::function<variant(const fc::variants &)> to_generic(
                    const std::function<api<Interface, Adaptor>(Args...)> &f) const;

            template<typename Interface, typename Adaptor, typename ... Args>
            std::function<variant(const fc::variants &)> to_generic(
                    const std::function<fc::optional<api<Interface, Adaptor>>(Args...)> &f) const;
    ...
    public:
        ...
        virtual result_type send_call(api_id_type api_id, std::string method_name,
        ...
        result_type receive_call(api_id_type api_id, const std::string &method_name,
    ...
    private:
        ...
        struct api_visitor {
            ...
            api_visitor(uint32_t api_id, std::shared_ptr<fc::binary_api_connection> con) : _api_id(api_id),
            ...
            static Result from_vector(const vector<char> &v, Result *,
            ...
            static fc::api_ptr from_vector(const vector<char> &v, fc::api_ptr * /* used for template deduction */,
                                           const std::shared_ptr<fc::binary_api_connection> &con) {
            ...
            template<typename Signature>
            static result_type convert_callbacks(const std::shared_ptr<fc::binary_api_connection> &con,
    ...
    generic_api::api_visitor::to_generic(const std::function<fc::api<Interface, Adaptor>(Args...)> &f) const {
    ...
    generic_api::api_visitor::to_generic(const std::function<fc::api_ptr(Args...)> &f) const {
        auto api_con = _api_con;
        auto gapi = &api;
    ...
    generic_api::api_visitor::to_generic(const std::function<void(Args...)> &f) const {
        generic_api *gapi = &api;
        return [f, gapi](const params_type &args) {
            fc::raw::datastream<const char *> ds(args.data(), args.size());
