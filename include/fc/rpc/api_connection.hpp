...
namespace fc {
    ...
    namespace detail {
        ...
        public:
            ...
            callback_functor(std::weak_ptr<fc::api_connection> con, uint64_t id) : _callback_id(id),
                    _api_connection(con) {
            }
        ...
        public:
            any_api(api_id_type api_id, const std::shared_ptr<fc::api_connection> &con) : _api_id(api_id),
                    _api_connection(con) {
            }
    ...
    private:
        ...
        R call_generic(const std::function<R(std::function<Signature>, Args...)> &f, variants::const_iterator a0,
                       variants::const_iterator e) {
            FC_ASSERT(a0 != e, "too few arguments passed to method");
            detail::callback_functor<Signature> arg0(get_connection(), a0->as<uint64_t>());
            return call_generic<R, Args...>(
                    this->bind_first_arg<R, std::function<Signature>, Args...>(f, std::function<Signature>(arg0)),
                    a0 + 1, e);
        }
        ...
        R call_generic(const std::function<R(Arg0, Args...)> &f, variants::const_iterator a0,
            ...
            FC_ASSERT(a0 != e, "too few arguments passed to method");
            return call_generic<R, Args...>(
                    this->bind_first_arg<R, Arg0, Args...>(f, a0->as<typename std::decay<Arg0>::type>()), a0 + 1, e);
        }
        ...
        struct api_visitor {
            ...
            template<typename Interface, typename Adaptor, typename ... Args>
            std::function<variant(const fc::variants &)> to_generic(
                    const std::function<fc::optional<api<Interface, Adaptor>>(Args...)
        ...
        std::vector<std::function<variant(const variants &)
    ...
    public:
        ...
        variant receive_call(api_id_type api_id, const std::string &method_name,
    ...
    private:
        ...
        struct api_visitor {
            ...
            api_visitor(uint32_t api_id, std::shared_ptr<fc::api_connection> con) : _api_id(api_id),
            ...
            static fc::api_ptr from_variant(const variant &v, fc::api_ptr * /* used for template deduction */,
                                            const std::shared_ptr<fc::api_connection> &con) {
                if (v.is_null()) {
                    return fc::api_ptr();
                }
            ...
            template<typename Signature>
            static fc::variant convert_callbacks(const std::shared_ptr<fc::api_connection> &con,
    ...
    template<typename R, typename ... Args>
    std::function<variant(const fc::variants &)> generic_api::api_visitor::to_generic(
            const std::function<R(Args...)> &f) const {
