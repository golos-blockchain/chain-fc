...
namespace fc {
    ...
    namespace http {
        ...
        struct header {
            header(std::string k, std::string v) : key(fc::move(k)), val(fc::move(v)) {
            }
        ...
        public:
            ...
            http::reply request(const std::string &method, const std::string &url,
                                const std::string &body = std::string(), const headers & = headers());
