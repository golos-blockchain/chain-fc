...
namespace fc {
    ...
    url::url(const std::string &u) : my(std::make_shared<detail::url_impl>()) {
        my->parse(u);
    }
    ...
    url::url(url &&u) : my(fc::move(u.my)) {
        u.my = get_null_url();
    }

    url::url(const mutable_url &mu) : my(std::make_shared<detail::url_impl>(*mu.my)) {
