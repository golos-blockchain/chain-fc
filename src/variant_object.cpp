...
namespace fc {
    ...
    variant_object::variant_object(string key, variant val) : _key_value(std::make_shared<std::vector<entry>>()) {
    ...
    variant_object::variant_object(variant_object &&obj) : _key_value(fc::move(obj._key_value)) {
        obj._key_value = std::make_shared<std::vector<entry>>();
        assert(_key_value != nullptr);
    }

    variant_object::variant_object(const mutable_variant_object &obj) : _key_value(
            std::make_shared<std::vector<entry>>(*obj._key_value)) {
    }

    variant_object::variant_object(mutable_variant_object &&obj) : _key_value(fc::move(obj._key_value)) {
        assert(_key_value != nullptr);
    }
    ...
    mutable_variant_object::mutable_variant_object(string key, variant val) : _key_value(new std::vector<entry>()) {
        _key_value->push_back(entry(fc::move(key), fc::move(val)));
    }

    mutable_variant_object::mutable_variant_object(const variant_object &obj) : _key_value(
            new std::vector<entry>(*obj._key_value)) {
    }

    mutable_variant_object::mutable_variant_object(const mutable_variant_object &obj) : _key_value(
            new std::vector<entry>(*obj._key_value)) {
    }

    mutable_variant_object::mutable_variant_object(mutable_variant_object &&obj) : _key_value(
