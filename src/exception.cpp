...
namespace fc {
    ...
    exception::exception(log_messages &&msgs, int64_t code, const std::string &name_value,
                         const std::string &what_value) : my(new detail::exception_impl()) {
        my->_code = code;
        my->_what = what_value;
        my->_name = name_value;
    ...
    exception::exception(const log_messages &msgs, int64_t code, const std::string &name_value,
                         const std::string &what_value) : my(new detail::exception_impl()) {
        my->_code = code;
        my->_what = what_value;
        my->_name = name_value;
    ...
    unhandled_exception::unhandled_exception(log_message &&m, std::exception_ptr e) : exception(fc::move(m)) {
        _inner = e;
    }
    ...
    exception::exception(int64_t code, const std::string &name_value, const std::string &what_value) : my(
    ...
    exception::exception(const exception &c) : my(new detail::exception_impl(*c.my)) {
    }
    ...
    int64_t exception::code() const throw() {
        return my->_code;
    }
    ...
    const log_messages &exception::get_log() const {
        return my->_elog;
    }
    ...
    void record_assert_trip(const char *filename, uint32_t lineno, const char *expr) {
        fc::mutable_variant_object assert_trip_info = fc::mutable_variant_object()("source_file", filename)(
                "source_lineno", lineno)("expr", expr);
