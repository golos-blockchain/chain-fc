...
namespace fc {
    namespace rpc {
        ...
        websocket_api_connection::websocket_api_connection(fc::http::websocket_connection &c) : _connection(c) {
            ...
            _connection.on_message_handler([&](const std::string &msg) {
                on_message(msg, true);
            });
            _connection.on_http_handler([&](const std::string &msg) {
                return on_message(msg, false);
            });
        ...
        variant websocket_api_connection::send_call(api_id_type api_id, std::string method_name,
        ...
        std::string websocket_api_connection::on_message(const std::string &message, bool send_message /* = true */ ) {
            std::string tmp("");
            _connection.rules.update_time();

            if (_connection.rules.check()) {
                tmp = on_message_impl(message, send_message);
                _connection.rules.increment();
            }

            _connection.rules.update_limit();


            return tmp;
        }

        std::string websocket_api_connection::on_message_impl(const std::string &message, bool send_message) {
            wdump((message));
            try {
                ...
                if (var_obj.contains("method")) {
                    ...
                    if (optexcept) {
                        ...
                        auto reply = fc::json::to_string(response(*call.id,
                                                                  error_object{1, optexcept->to_detail_string(),
