...
variant websocket_api_connection::send_call(api_id_type api_id, std::string method_name, variants args /* = variants() */ ) {
...
variant websocket_api_connection::send_call(std::string api_name, std::string method_name, variants args ) {
...
void websocket_api_connection::send_notice(uint64_t callback_id, variants args /* = variants() */ ) {
    fc::rpc::request req;
    req.jsonrpc = "2.0";
    req.id = optional<uint64_t>();
    req.method = "notice";
    req.params = {callback_id, std::move(args)};
