        ...
        switch (v.get_type()) {
            case object_type:
                *reinterpret_cast<variant_object **>(this) = new variant_object(
                ...
                break;
            case array_type:
                *reinterpret_cast<variants **>(this) = new variants(
        ...
        switch (get_type()) {
            case string_type: {
                ...
                FC_THROW_EXCEPTION(bad_cast_exception,
                                   "Cannot convert std::string to bool (only \"true\" or \"false\" can be converted)");
            }
            case double_type:
    ...
    std::string variant::as_string() const {
        switch (get_type()) {
            ...
            case null_type:
                return std::string();
            default:
                FC_THROW_EXCEPTION(bad_cast_exception, "Invalid cast from ${type} to std::string", ("type", get_type()));
        }
    }
