...
namespace fc {
    ...
    gelf_appender::gelf_appender(const variant &args) : my(new impl(args.as<config>())) {
        try {
            try {
            ...
            if (!my->gelf_endpoint) {
                ...
                std::string::size_type colon_pos = my->cfg.endpoint.find(':');
                try {
                    uint16_t port = boost::lexical_cast<uint16_t>(
                            my->cfg.endpoint.substr(colon_pos + 1, my->cfg.endpoint.size()));

                    std::string hostname = my->cfg.endpoint.substr(0, colon_pos);
    ...
    void gelf_appender::log(const log_message &message) {
        ...
        if (gelf_message_as_string.size() <= max_payload_size) {
            // no need to split
            std::shared_ptr<char> send_buffer(new char[gelf_message_as_string.size()], [](char *p) {
                delete[] p;
            });
            ...
            while (bytes_sent < gelf_message_as_string.size()) {
                ...
                std::shared_ptr<char> send_buffer(new char[max_payload_size], [](char *p) {
                    delete[] p;
                });
                ...
                *(unsigned char *) (ptr++) = total_number_of_packets;
                ...
                my->gelf_socket.send_to(send_buffer, header_length + bytes_to_send, *my->gelf_endpoint);
