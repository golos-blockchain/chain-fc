...
namespace fc {
    ...
    log_context::log_context(log_level ll, const char *file, uint64_t line, const char *method) : my(
    ...
    void from_variant(const variant &v, log_level &e) {
        ...
        } FC_RETHROW_EXCEPTIONS(error, "Expected 'all|debug|info|warn|error|off', but got '${variant}'",
                                ("variant", v));
    }
    ...
    variant log_context::to_variant() const {
        ...
        o("level", variant(my->level))("file", my->file)("line", my->line)("method", my->method)("hostname",
                                                                                                 my->hostname)(
    ...
    log_message::log_message(log_context ctx, std::string format, variant_object args) : my(
            std::make_shared<detail::log_message_impl>(std::move(ctx))) {
        my->format = std::move(format);
        my->args = std::move(args);
    }

    log_message::log_message(const variant &v) : my(
            std::make_shared<detail::log_message_impl>(log_context(v.get_object()["context"]))) {
